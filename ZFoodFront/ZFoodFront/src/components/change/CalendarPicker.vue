<template>
  <div class="datepicker" :class="{'datepicker-range':range,'datepicker__clearable':clearable&&text&&!disabled}">
    <!--<span style="margin-left: 10px;">{{startTime}}<span v-if="startTime!==null&&endTime!==null">~</span>{{endTime}}</span>-->
    <input readonly :value="dateText" :class="[show ? 'focus' : '', inputClass]" :disabled="disabled" :placeholder="placeholder" :name="name" v-if="type!=='inline'" />
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      text () {
        ...
        /*if (this.noList) {
         ...
         console.log('text1');
         return this.dates.map(date => this.tf(date)).join(` ${this.rangeSeparator} `);
         }
      ...
      startTime: {
        handler: function (val, oldval) {
          if(val===null)
            ...
            this.dateText=this.tf(new Date(val))+'~'+this.tf(new Date(this.endTime));
          //-00
          /* if (this.index === 0)
           console.log(val + ' ' + oldval + ' ' + this.index);*/
          /* if(val===null) {
           this.dates = [new Date(),new Date()];
           }
           else
           this.dates = [new Date(val), new Date(this.endTime)];*/
        },
        deep:true
      },
      endTime: {
        handler: function (val, oldval) {
          if(val===null)
            ...
            this.dateText=this.tf(new Date(this.startTime))+'~'+this.tf(new Date(val))
          //-00
          /*if(val===null) {
           this.dates = [new Date(), new Date()];
           }
           else
           this.dates = [new Date(this.startTime), new Date(val)];*/
        },
        deep:true
      },
      dates(val){
        this.changeDate=[val[0],val[1]];
        this.dateText=this.tf(new Date(this.startTime))+'~'+this.tf(new Date(val));
        this.$emit('change', this.tf(this.dates[0]),this.tf(this.dates[1]),this.index);//不能加在text中会引起显示数据错误
      }
      //NHVZG-e
    },
    methods: {
    ...
    mounted () {
      if(this.startTime!==null&&this.endTime!==null)
        this.dateText=this.tf(new Date(this.startTime))+'~'+this.tf(new Date(this.endTime));
